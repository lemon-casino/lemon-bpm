 ...
 * 工时设置结构定义
 */
export type WorkTimeHandler = {
  // 是否按工作时间计算超时
  workTimeEnable: boolean
  // 工作时间类型
  workTimeType?: number
  // 工时时间
  timeDuration?: string
...
NODE_DEFAULT_TEXT.set(NodeType.HTTP_TASK_NODE, '请设置HTTP任务')
...
NODE_DEFAULT_NAME.set(NodeType.HTTP_TASK_NODE, 'HTTP任务')
...
export enum ProcessVariableEnum {
  ...
  //链接ID
  HTTP_PROCESS_INSTANCE_ID='HTTP_PROCESS_INSTANCE_ID'
}
...
export enum BpmHttpRequestMethodEnum {
  /**
   * GET 请求
   */
  GET = 1,
...
export type HttpRequestSetting = {
  ...
  refreshId?: string
  // 请求方法
  method?: BpmHttpRequestMethodEnum
  // 请求路径
  url?: string
  // 请求头参数设置
  header?: HttpRequestParam[]
