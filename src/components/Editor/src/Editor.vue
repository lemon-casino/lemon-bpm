  ...
  return Object.assign(
    {
      ...
      autoFocus: false,
      scroll: true,
      // 添加默认的EXTEND_CONF配置，确保mentionConfig存在
      EXTEND_CONF: {
        mentionConfig: {
          showModal: () => {
            console.log('默认的showModal被调用，但未实现具体功能');
          },
          hideModal: () => {
            console.log('默认的hideModal被调用，但未实现具体功能');
          }
        }
      },
      MENU_CONF: {
 ...
 */
const saveSelection = () => {
  if (editorRef.value && typeof editorRef.value.saveSelection === 'function') {
...
const restoreSelection = () => {
  if (editorRef.value && typeof editorRef.value.restoreSelection === 'function') {
