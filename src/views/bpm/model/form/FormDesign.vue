<template>
  <el-form ref="formRef" :model="modelData" :rules="rules" label-width="120px" class="mt-20px">
    <el-form-item label="表单类型" prop="formType" class="mb-20px">
      <el-radio-group v-model="modelData.formType" :disabled="props.isPreviewOnly">
        <el-radio
...
watch(
  ...
  async (newFormId) => {
    if (newFormId && modelData.value.formType === BpmModelFormType.NORMAL) {
      // 优先使用最新的表单数据（如果存在）
      if (props.latestFormData && props.latestFormData.id === newFormId) {
        console.log('使用最新的表单数据进行预览:', props.latestFormData)
        setConfAndFields2(formPreview.value, props.latestFormData.conf, props.latestFormData.fields)
      } else {
        // 没有最新数据，从API获取
